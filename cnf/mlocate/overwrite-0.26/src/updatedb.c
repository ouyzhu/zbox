...
// by ouyzhu: osx NOT support gnulib <mntent.h>, comment for osx 
//#include <mntent.h>
#include "error.h"
      ...
      const char *relative)
{
  ...
  if (lstat (relative, &st) != 0)
    goto err;
  // by ouyzhu: osx NOT support gnulib <mntent.h>, comment for osx, which means ignore the config item "prunefs"
